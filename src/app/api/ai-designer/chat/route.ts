...
const CATEGORY_STYLE_GUIDE: Record<string, string> = {
  "חתונה":
    "אלגנטי ורומנטי: זהב/שמפניה/קרם/לבן, גווני בורדו או נייבי כאקצנט, רקעים כמו הינומה מתנופפת, ים/שקיעה, פרחים לבנים, טקסטורת שיש. אופציה חזקה נוספת שכדאי להציע: רקע צילומי לילה של מסדרון/חופה מואר בפמוטים ונורות חוט, מסגרת דקורטיבית של פרחים לבנים ונורות שעוטפת את כל הכרטיס, טבעות זהב משולבות כמוטיב מעל הכותרת, כותרת מוזהבת מטאלית עם ברק, ופרטי האירוע כשורות אייקון+טקסט קטנות (לוח שנה, מיקום, טבעות) מופרדות בקווי זהב דקים.",
  "בר/בת מצווה":
...
function buildSystemInstruction(eventCategory: string | undefined, categoryFields: Record<string, string> | undefined) {
  ...
  return [
    ...
    "כשאתה מוכן (readyToGenerate=true), כתוב ב-imagePrompt פרומפט מלא באנגלית ליצירת ההזמנה השלמה כתמונה אחת מוגמרת - לא רק רקע. חובה לכלול הנחיה מפורשת שכל הטקסט הבא מעוצב ומשולב בתוך התמונה עצמה, באותיות עבריות אלגנטיות (לא תרגום לאנגלית, הטקסט חייב להיות בעברית בדיוק כמו שקיבלת אותו): שם/שמות החוגגים בגדול ובולט במרכז, תאריך האירוע, שמות ההורים אם יש, מקום האירוע וכתובת אם יש. תן לו הנחיה מפורטת על ההיררכיה הוויזואלית (מה גדול, מה קטן, סדר השורות), טיפוגרפיה מוזהבת/אלגנטית, ורקע מתאים לסגנון/צבעים שהמשתמש בחר (למשל רקע ים, פרחים, מרקם יוקרתי וכו' לפי מה שהוא ענה). ציין את **יחס הגובה-רוחב** כפורטרט גבוה במיוחד, בערך 9:19.5 (גבוה משמעותית מ-9:16 הרגיל) - זה קרוב בהרבה למידות המסך האמיתיות של טלפון מודרני, כך שההזמנה תמלא את המסך מקצה לקצה בלי רצועות ריקות למעלה או למטה. תוסיף גם הנחיה מפורשת שכל טקסט (שמות, תאריך, שעה, מקום, שמות הורים) וגם המסגרת הדקורטיבית חייבים להישאר במרחק בטוח וגדול מכל ארבעת הקצוות של התמונה - לפחות 8% מרוחב/גובה התמונה כשוליים ריקים מכל צד - כי התמונה תוצג עם מילוי מלא של המסך (ללא רצועות ריקות בצדדים), כך שמכשיר עם יחס מסך מעט שונה עלול לחתוך פס דק בקצוות, ואסור שהחיתוך הזה יפגע בטקסט. לעולם אל תזכיר 'טלפון', 'מסך', 'טיקטוק' או 'סטורי' בתור השראה ויזואלית, כי זה גורם למודל לצייר בטעות מסגרת טלפון/notch/סמלי אפליקציה כחלק מהתמונה עצמה. תוסיף במפורש: no phone frame, no device mockup, no screen bezel, no notch, no app UI elements anywhere in the image - a full-bleed flat graphic design only. קריטי לגבי איות: תוסיף גם משפט מפורש שמזהיר שכל מילה עברית חייבת להיות מאויתת באופן מדויק, אות-אות, בדיוק כמו שסופקה - למשל: 'Spell every Hebrew word with perfect, exact accuracy - copy each name/date/venue letter by letter exactly as given, do not invent, merge, drop, or add any letters.' חשוב באותה מידה: תוסיף גם הנחיה מפורשת שאסור להוסיף שום טקסט, מילה, כותרת-משנה או תווית שלא נתת לו את הנוסח המדויק שלה - כלומר אם לא ביקשת ממנו במפורש תווית 'בר/בת מצווה' או משפט פתיחה כלשהו, שלא ימציא כזה בעצמו - למשל: 'Do not add any text, label, subtitle, or greeting beyond exactly what is specified above - no invented category labels, no invented opening phrases.'",
    ...
    "לגבי גודל שם/שמות החוגגים: זו חייבת להיות שורת הטקסט **הגדולה והעבה ביותר בכל הכרטיס ללא יוצא מן הכלל** - משמעותית יותר גדולה מכל טקסט אחר (תאריך, מקום, שעות), תופסת נתח משמעותי מגובה התמונה, לא רק 'בולטת יחסית'. תוסיף את זה במפורש ל-imagePrompt באנגלית, למשל: 'The celebrant name(s) must be the single largest and boldest text element on the entire card - dramatically larger than every other line, occupying significant vertical space, extra bold weight, the unmistakable visual focal point.'",
